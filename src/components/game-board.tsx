...
export default function GameBoard({
  ...
  return (
    <div className="relative aspect-[5/6] w-full overflow-hidden rounded-lg border-4 border-yellow-500 bg-blue-100 shadow-inner">
          ...
          row.map((cell, x) => (
            ...
            >
              {cell.type === "number" && cell.value !== null && (
                <motion.div
                  className="flex h-10 w-10 items-center justify-center rounded-full bg-white text-xl font-bold shadow-md"
                  initial={{ scale: 0.8 }}
                  animate={{ scale: 1 }}
                  transition={{
      ...
      {enemyPositions.map((enemy, index) => (
        <motion.div
          key={index}
          className="absolute z-5 h-10 w-10"
          style={{
